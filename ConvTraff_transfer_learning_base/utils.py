...
def get_dataset(pickle_filename, args, parser):
    if os.path.exists(pickle_filename):
        ...
        train_set = np.load('train_set_tl.npy')
        train_labels = np.load('train_labels_tl.npy')
    else:
        ...
        np.save('train_set_tl.npy', train_set)
        np.save('train_labels_tl.npy', train_labels)
...
def compile_and_fit(model, train_set,train_labels,valid_set, valid_labels, initial_learning_rate, decay_steps, 
    ...
    csv_logger = keras.callbacks.CSVLogger('logs/ConvTraff_transfer_learning_base.csv',append =True)

    checkpoint_filepath = 'savedModel/ConvTraff_transfer_learning_base/model.{epoch:02d}.ckpt'
...
def plot_prediction(real_data, prediction,dataset_len,test_len):
    ...
    for i in range(real_data.shape[-1]):
        ...
        plt.savefig('Images/ConvTraff_transfer_learning_base/Grafica'+str(dataset_len)+'_'+str(test_len)+'_' + str(i) + '.png')